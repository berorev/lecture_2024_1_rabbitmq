...
import com.fasoo.lecture.rabbitmqchat.lecture5.error.CustomErrorHandler;
import com.fasoo.lecture.rabbitmqchat.lecture5.error.CustomFatalExceptionStrategy;
...
public class DataSenderConfiguration {
  ...
  public Queue myUserQueue() {
    return QueueBuilder.durable("user." + Lecture5Config.USERNAME)
        .deadLetterExchange("")
  ...
  public Declarables topicBindings(
    ...
    FanoutExchange userOneExchange = new FanoutExchange("user." + Lecture5Config.USERNAME);

    return new Declarables(
        userOneExchange,

        BindingBuilder.bind(myUserQueue()).to(userOneExchange),
        BindingBuilder.bind(userOneExchange)
            .to(userTopicExchange)
            .with("*.user." + Lecture5Config.USERNAME));
  }
}
